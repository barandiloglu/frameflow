  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <header className="shrink-0 flex items-center justify-between px-8 py-5 border-b border-ivory-05">
        <div>
          <h1 className="font-display font-bold text-xl text-ivory">Dashboard</h1>
          <p className="text-sm font-body text-ivory-30 mt-0.5">Welcome back, Acadia Bakes</p>
        </div>
        ...
        <div className="grid gap-4" style={{ gridTemplateColumns: "1.4fr 1fr" }}>
          ...
          <div className="rounded-[2px] border border-ivory-05 bg-ivory-05/50 overflow-hidden">
            <div className="px-5 py-4 border-b border-ivory-05">
              <h2 className="font-display font-semibold text-sm text-ivory">Upcoming Posts</h2>
            </div>
            <div className="divide-y divide-ivory-05">
              {posts.map((p, i) => (
                <div key={i} className="flex items-center gap-4 px-5 py-3.5">
                  {/* Thumbnail placeholder */}
                  <div className="w-10 h-10 rounded-[2px] bg-ivory-05 shrink-0 flex items-center justify-center overflow-hidden">
                    <svg width="20" height="20" viewBox="0 0 20 20" className="text-ivory-10">
                      <defs>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-0.5">
                      ...
                      <span className="text-[10px] font-body text-ivory-30">{p.platform}</span>
                    </div>
                    <p className="text-sm font-body text-ivory truncate">{p.title}</p>
                  </div>
                  {/* Date */}
                  <span className="text-xs font-body text-ivory-30 shrink-0">{p.date}</span>
                  {/* Status */}
          ...
          <div className="rounded-[2px] border border-ivory-05 bg-ivory-05/50 overflow-hidden">
            <div className="px-5 py-4 border-b border-ivory-05">
              <h2 className="font-display font-semibold text-sm text-ivory">Activity</h2>
            </div>
            <div className="divide-y divide-ivory-05">
              {activities.map((a, i) => (
                <div key={i} className="px-5 py-3.5 flex items-start gap-3">
                  <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-ivory-10 shrink-0" />
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-body text-ivory leading-snug">{a.text}</p>
                    <p className="text-[11px] font-body text-ivory-30 mt-0.5">{a.time}</p>
                  </div>
                </div>
              ))}
            </div>
