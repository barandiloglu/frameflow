...
const activities = [
  ...
  { client: "Velour Collective", description: "Monthly report generated", time: "Yesterday", dotColor: "bg-on-surface" },
  ...
  { client: "Birchfield Co.", description: "Social media onboarding started", time: "3 days ago", dotColor: "bg-on-surface" },
];

const plans = [
  { name: "Growth Plan", count: 5, width: "42%", barColor: "bg-amber" },
  { name: "Starter Plan", count: 4, width: "33%", barColor: "bg-on-surface-30" },
  { name: "Enterprise", count: 3, width: "25%", barColor: "bg-on-surface-30" },
];
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-surface">
      ...
      <header className="shrink-0 h-[68px] flex items-center justify-between px-8 border-b border-border-subtle">
        <div>
          <h1 className="font-display font-bold text-xl text-on-surface">Admin Dashboard</h1>
          <p className="text-sm font-body text-on-surface-30 mt-0.5">Manage your clients and projects</p>
        </div>
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" className="absolute left-2.5 top-1/2 -translate-y-1/2 text-on-surface-30">
            ...
            <input
              type="text"
              placeholder="Search..."
              className="w-[200px] h-9 pl-8 pr-3 text-sm font-body text-on-surface placeholder:text-on-surface-30 bg-on-surface-05 border border-on-surface-10 rounded-[2px] outline-none focus:border-on-surface-30 transition-colors"
            />
          </div>
          {/* Avatar */}
        ...
        <div className="grid gap-4" style={{ gridTemplateColumns: "1.5fr 1fr" }}>
          ...
          <div className="rounded-[2px] border border-border-subtle bg-on-surface-05 overflow-hidden">
            <div className="px-5 py-4 border-b border-border-subtle flex items-center justify-between">
              <h2 className="font-display font-semibold text-sm text-on-surface">Recent Client Activity</h2>
              ...
              {activities.map((a, i) => (
                <div key={i} className="flex items-center gap-3 px-5 py-3.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-body font-semibold text-on-surface">{a.client}</p>
                    <p className="text-xs font-body text-on-surface-60 mt-0.5">{a.description}</p>
                  </div>
                  <span className="text-xs font-body text-on-surface-30 shrink-0">{a.time}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-[2px] border border-border-subtle bg-on-surface-05 overflow-hidden">
            <div className="px-5 py-4 border-b border-border-subtle">
              <h2 className="font-display font-semibold text-sm text-on-surface">Clients by Plan</h2>
            </div>
            <div className="px-5 py-4 space-y-4">
              {plans.map((p) => (
                <div key={p.name}>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm font-body text-on-surface">{p.name}</span>
                    <span className="text-xs font-body text-on-surface-30">{p.count} clients</span>
                  </div>
                  <div className="w-full h-2 rounded-full bg-on-surface-05">
                    <div
            ...
            <div className="px-5 py-4 border-t border-border-subtle">
              <p className="text-[10px] font-body font-semibold tracking-[0.15em] text-on-surface-30 uppercase mb-3">
                Quick Actions
              </p>
              <div className="space-y-2">
                ...
                <button className="w-full h-9 rounded-[2px] bg-transparent border border-on-surface-10 text-on-surface-60 text-sm font-body hover:bg-on-surface-05 transition-colors">
                  Create Project
                </button>
                <button className="w-full h-9 rounded-[2px] bg-transparent border border-on-surface-10 text-on-surface-60 text-sm font-body hover:bg-on-surface-05 transition-colors">
                  Generate Report
